...
public interface OrderMatching {
    ...
    default Report processConditions(List<MatchingCondition> conditions, OrderBook orderBook, Order orderCandidate, Order order) {
        for (MatchingCondition condition : conditions) { // Iterate over all conditions and return the first valid Report
            Report processResult = condition.process(orderBook, orderCandidate, order);
